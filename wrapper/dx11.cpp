...
public: // Implementation
	...
	virtual void copyDepthStencilViewTarget(const std::string & name, const DepthStencilView & view_id) final {
		// TIC;
		// LOG(INFO) << "bla 1";
...
#undef READ_TARGET
	...
	virtual bool unpackDepthTarget(const std::string& name, int W, int H, int C, unsigned char* depth, unsigned char* stencil)
	{
		auto t = targets_.find(name); 
		if (t != targets_.end())
			return t->second->unpackDepthStencil(W, H, C, depth, stencil);
		LOG(WARN) << "Target '" << name << "' not found!"; 
		return false; 
	}
