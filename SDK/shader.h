...
public:
	...
	static IMPORT std::shared_ptr<Shader> create(const std::vector<uint8_t>  & src, const std::unordered_map<std::string, std::string> & name_remap = std::unordered_map<std::string, std::string>());
